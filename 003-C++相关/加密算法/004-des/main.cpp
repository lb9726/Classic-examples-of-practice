...
{
	if(argc < 2 || argv[1][0] != '-')
		help("Command Args Error1") ;//输入参数错误，打印帮助文件然后退出
	...
	switch(argv[1][1])
	{
		...
		case 'h'://打印帮助文件
			...
			break ;
		default:
			help("Command Args Error2") ;
			break ;
	}
	if(argc < 5 || strlen(argv[4]) < 8)
		help("Command Args Error3") ;//输入参数错误，打印帮助文件然后退出

	ifstream srcFile(argv[2]) ;//输入文件
	ofstream tarFile(argv[3]) ;//输出文件
	if(!srcFile || !tarFile) help("File Open Error4") ;//文件打开失败
