...
void PrintInfoIntoTable1(char *array[],char *str[],int max[],int a,int n);
...
void ChooseColLinking(char *array[],char *str[],int a,int n);
...
void DefineAarryandStorageInfo()
{
	...
	PrintInfoIntoTable1(name,str,max,a,n);
...
void AddBlank(char *array[], int *length[],int c,int n)
{
	...
	for( i = 0; i < c; i++)
	{
		for(k = 0; k < *length[i] + 4; k++)//+4是想增长一些宽度，打印时好看一点
		{
			strcat(blank," ");
		}
...
//模拟数据库中只选择某些列中的内容打印连接|和字符串中的某些列
void ChooseColLinking(char *array[],char *str[],int a,int n)
{
	int i = 0;
	int j = 0;
...
void PrintInfoIntoTable1(char *array[],char *str[],int max[],int a,int n)
{
	...
	int j = 0;
	int k = 0;
	ChooseColLinking(array,str,a,n);
