...
void generate_node(WORKERS people,STNODE *head)
{
...
void read_file_use_fscanf(char filename[30],STNODE *head)
{
	...
	while(!feof(file))
    {
        ...
        generate_node(worker,head);//利用尾插法将文件中读取的信息临时存储在链表中
    }//end of while
    file = NULL;
}
...
void ReadInfotoArray(char *array[],int i)
{
	...
	while(p != NULL)
	{
		
		{
			...
			strcpy(array[i],p->person.Num);  i++;
			strcpy(array[i],p->person.Sex);  i++;
			strcpy(array[i],p->person.Age);  i++;
			strcpy(array[i],p->person.Rank); i++;
			strcpy(array[i],p->person.Department); i++;
			strcpy(array[i],p->person.Salary); i++;
			strcpy(array[i],p->person.Passwd); i++;
		}printf("i=%d\n",i);      
		p = p->next;
	}
