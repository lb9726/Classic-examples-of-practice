...
/******************************************
功能:manager增删改查的操作的选项
使用:此函数在manager登录成功时调用
传入参数:登录成功者的姓名
******************************************/
void manageraction(char name[30],char pos[30])
{
	STNODE *head1=NULL;
	...
	while(choice1)
	{
		choice=ManagerUI(name);//managerUI层的调用
		switch(choice)
		{
			case 1:
...
****************************************/
void managerlogin(STNODE *head)//manager登录验证函数
{
	int i;
	char sname[20];
	char pass[30];
	for(i=1;i<4;i++)
	{
		...
		while(p!=NULL)
		{ 
			if((strcmp(sname,p->person.name)==0&&strcmp(pass,p->person.passwd)==0)&&strcmp("经理",p->person.position)==0)
			{     
				if(iputchecking())//验证码输入为正确的
				{
					logininf(p->person.id,p->person.name,p->person.passwd);
					manageraction(p->person.name,p->person.position);//调用经理操作函数
					break;
				}
			}
		    p=p->next;
