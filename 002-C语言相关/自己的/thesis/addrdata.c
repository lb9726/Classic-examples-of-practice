...
{
	char *verify_code = (char *)malloc(sizeof(char *));
...
{   
	...
	char *check_code = (char *)malloc(sizeof(char *));
    while(i < 3)
    {//printf("3333\n");
    	//s = get_rand_str(4);
    	//printf("get_rand_str end!!\n");
...
void logininf(char name[30],char pass[30])
{
	...
	if(p == NULL)
	{
		...
		write_file("logintable.txt",person);
		sleep(1);
		return;
	}
	else
	{
		...
		write_file("logintable.txt",p->person1);
		sleep(1);
		return;
	}
}
